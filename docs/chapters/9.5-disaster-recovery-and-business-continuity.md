...
- **Level 1, Initial:** Backups may run but restores are untested. There are no agreed RTO or RPO, no business impact analysis, and recovery is improvised during the incident. A serious data loss or ransomware event would likely be unrecoverable.
- **Level 2, Managed:** Critical systems have backups following the 3-2-1 rule and documented RTO and RPO for the most important services. A basic DR plan exists and restores are tested occasionally, but coverage is partial, dependencies are not mapped, and drills are ad hoc.
- **Level 3, Defined:** A business impact analysis drives tiered RTO and RPO across systems. Recovery strategies are matched to tiers, environments are infrastructure as code, dependencies are mapped, and scheduled drills (tabletop, game day, and failover) measure actual against target. A cyber-recovery plan with immutable copies exists.
- **Level 4, Optimizing:** Recovery is continuously exercised and measured, with actual RTO and RPO tracked as reliability metrics feeding improvement. Cyber-recovery clean-room restores are rehearsed, supplier and concentration risk is actively managed, and continuity is integrated with reliability (chapter 9.1) and incident response (chapter 9.3) so the organization recovers predictably from failures it has never seen.
