...
- **Multi-agent theater.** Splitting a simple task across agents, paying coordination cost for no gain.
...
- **Level 1, Initial:** Agents are prototyped ad hoc with broad tool access and no bounds. Success is judged by demos. There is no evaluation set, no tracing, and no human gate on consequential actions.
- **Level 2, Managed:** Some agents have bounded loops and least-privilege tools. Basic tracing exists and a manual evaluation set catches gross regressions. Human approval guards the most obvious irreversible actions, but coverage is uneven.
- **Level 3, Defined:** Shared patterns govern autonomy, tool permissions, sandboxing, and human-in-the-loop gates. Every consequential action is gated or validated. Agents are traced end to end, and an automated evaluation set with success-rate scoring runs on every change. Prompt injection is treated as a standing threat.
- **Level 4, Optimizing:** Autonomy is matched to task risk by policy. Continuous offline and online evaluation ties agent behavior to business outcomes. Tracing, cost budgets, and audit trails are uniform across the portfolio; injection and confused-deputy defenses are tested adversarially; accountability for autonomous actions is clear and auditable.
