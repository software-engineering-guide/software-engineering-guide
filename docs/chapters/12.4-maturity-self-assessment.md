...
3. Average scores within a  to see where a whole domain stands, then look at
   the spread: a  at "3 on average" that hides a Level 1 chapter still has a
   Level 1 risk.
...
##  10. Project/Product/Program Management
...
2. **Average by .** Sum the chapter scores in a  and divide by the number
   of chapters. This gives a per- maturity (e.g., " IV averages 2.5").
3. **Record the minimum, not just the mean.** A  that averages 3.0 but
...
A simple worksheet per :

|  | Chapters scored | Average (mean) | Lowest chapter | Notes / priority |
| --- | --- | --- | --- | --- |
