...
- **Level 1, Initial:** Builds are ad hoc scripts nobody owns, often run from developer machines. Output is nondeterministic, dependencies float without lockfiles, artifacts are rebuilt per environment and deployed by mutable tag, and there is no shared cache, no signing, and no bill of materials.
- **Level 2, Managed:** Builds run in CI from a checked-in definition. Dependencies are locked, artifacts land in a managed repository with basic versioning, and a local or simple remote cache speeds up common builds. Promotion exists in principle but rebuilds still happen, and provenance is inconsistent.
- **Level 3, Defined:** Builds are reproducible and largely hermetic, with pinned toolchains and a shared remote cache whose keys hash all real inputs. Artifacts are immutable, content-addressed, versioned semantically, built once and promoted everywhere, signed, and shipped with an SBOM. Cache access is controlled and retention policies are enforced.
- **Level 4, Optimizing:** Build performance and cache-hit rates are measured and continuously tuned, and developer feedback time is a tracked metric. Build-time supply-chain integrity meets a graded framework such as SLSA, provenance is verified at deploy time, and the organization refines its tooling, retention, and security posture as it learns from incidents and audits.
