...
- **Revocation:** invalidate a compromised key or certificate quickly, and make sure systems honor the revocation.
...
- **Level 1, Initial:** Encryption is inconsistent and often absent. Keys and secrets are hard-coded or shared informally. No inventory, no rotation, and certificates expire by surprise. Teams occasionally write their own crypto.
- **Level 2, Managed:** TLS and encryption at rest are enabled for major systems. A KMS or secrets manager exists but is used unevenly. Some certificates are monitored, but rotation is manual and rare, and there is no complete crypto inventory.
- **Level 3, Defined:** Encryption in transit and at rest is the default, keys live in a KMS with scheduled rotation and envelope encryption, and key custody is separated from data access. Certificate lifecycles are automated, a crypto inventory exists, and validated modules are used where required.
- **Level 4, Optimizing:** Cryptography is a platform capability every service inherits by default. Rotation and revocation are fast and rehearsed, HSMs protect the highest-assurance keys, and cryptographic agility plus an active post-quantum migration plan keep the estate ready for change. Compliance evidence is produced automatically.
