...
Give every standard an **owner** (a role, not just a person) accountable for keeping it current, and a **review cadence** (at least annually). Provide a lightweight path for anyone to propose a change through a pull request or an **[RFC](https://en.wikipedia.org/wiki/Request_for_Comments) (request for comments)**, a written proposal circulated for feedback before adoption. Version standards, deprecate them explicitly, and announce changes. A standards catalog that is never revised rots into folklore that people cite selectively and trust little.
